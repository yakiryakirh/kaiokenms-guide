(() => {
  ...
  const mapImage=m=>m?.image||`${API}/map/${m?.id}/minimap`;
  ...
  const style=document.createElement('style'); style.textContent=`
    ...
    .map-box{border:1px solid #35547c;border-radius:13px;padding:12px;margin:10px 0;background:#101b2b}.map-canvas{position:relative;overflow:hidden;border:1px solid #35547c;border-radius:11px;background:#0b1320;min-height:100px}.map-canvas>img{display:block;width:100%;max-height:440px;object-fit:contain}.marker{position:absolute;transform:translate(-50%,-50%);width:18px;height:18px;border-radius:50%;border:3px solid #fff;box-shadow:0 0 10px currentColor}.marker.portal{color:#ff9800;background:#ff9800}.marker.npc{color:#ff4d71;background:#ff4d71}.map-legend{font-size:.78rem;color:#b9c7da;margin-top:7px}.portal-list{display:grid;gap:7px;margin-top:9px}.portal-row{display:flex;justify-content:space-between;gap:10px;align-items:center;border:1px solid #35547c;border-radius:9px;padding:8px 10px}.portal-row button,.mini-open{border:1px solid #ff8a00;background:#6b3300;color:#ffd54a;border-radius:8px;padding:5px 9px;font-weight:900;cursor:pointer}
  ...
  function mapBox(rec,{npcPoints=[],showPortals=true}={}){
    ...
    return `<div class="map-box"><div><b>${esc(m.name)}</b>${m.street?` <span style="color:#9fb1c7">• ${esc(m.street)}</span>`:''}</div><div class="map-canvas"><img src="${esc(mapImage(m))}" alt="${esc(m.name)} map">${portals.map(p=>markerHTML('portal',p,`Portal to ${p.targetMapName}`)).join('')}${npcPoints.map(p=>markerHTML('npc',p,'NPC location')).join('')}</div><div class="map-legend">${portals.length?'● Portal':''}${npcPoints.length?' &nbsp; ★ NPC location':''}</div>${portals.length?`<div class="portal-list">${portals.map(p=>`<div class="portal-row"><span>To <b>${esc(p.targetMapName)}</b></span><button type="button" data-map="${esc(p.targetMapId)}">Open map</button></div>`).join('')}</div>`:''}</div>`;
  }
  function bindMapButtons(root=detailBody){root.querySelectorAll('[data-map]').forEach(b=>b.onclick=()=>{const m=findMap(b.dataset.map);if(m)openEntity('map',m)})}
  ...
  function openNpc(row){const maps=mapRecordsForNpc(row.id),quests=(idx.quests||[]).filter(q=>nkey(q.startNpc)===nkey(row.id)||nkey(q.endNpc)===nkey(row.id));showDetail(row.name,`<div style="display:flex;gap:18px;align-items:center"><img src="${icon('npc',row.id)}" style="width:110px;height:110px;object-fit:contain" alt=""><div><h2 style="margin:0">${esc(row.name)}</h2><div class="pre4-meta">${maps.length} mapped location${maps.length===1?'':'s'}</div></div></div>${quests.length?`<h3>Related Quests</h3><div class="entity-links">${quests.map(q=>linkCard('quest',q,[nkey(q.startNpc)===nkey(row.id)?'Starts here':'',nkey(q.endNpc)===nkey(row.id)?'Ends here':''].filter(Boolean).join(' • '))).join('')}</div>`:''}${maps.length?`<h3>NPC Locations</h3>${maps.map(x=>mapBox(x.m,{npcPoints:x.pts,showPortals:true})).join('')}`:'<p class="empty">No verified v83 map location is listed for this NPC.</p>'}`)}
